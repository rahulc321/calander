...
class UsersController extends Controller
{
    ...
    function user_info(){
        ...
        foreach($userInfos as $userInfo){
            ...
            // grand total
            $gtotal = Order::join('invoices','invoices.order_id','=','orders.id')
                ->select(\DB::raw('sum(price) as gtotal,shipping_price'))
                ->where('invoices.user_id',$userId)->first();
            //echo '<pre>';print_r($gtotal['shipping_price']);die;
            $allData['paid'][]=$oData;
            $allData['unpaid'][]=$oData1['total1']+$gtotal['shipping_price'];
            $allData['cancel'][]=$cancel['ctotal'];
            $allData['gtotal'][]=$gtotal['gtotal']+$gtotal['shipping_price'];
...
/*************************************************************/
    ...
    function user_data($id){
        ...
        $uData = Order::join('invoices','invoices.order_id','=','orders.id')->where('invoices.user_id',$id)->where('invoices.status',2)
        // ->groupBy('order_items.order_id')
        ->orderBy('invoices.id', 'DESC')->get();
        echo '<pre>';print_r($uData);die;
        // unpaid amount
        $unpaid = Order::join('invoices','invoices.order_id','=','orders.id')
                ->select(\DB::raw('sum(price) as total1,shipping_price'))
    ...
    public function sendmail(PdfExportService $exportService,Request $request){
       ...
       // echo '<pre>';print_r($request->all());
        ...
        if (is_null($invoice)) {
            ...
            $exportService
            ->setName('IID-'.$invoice->IID .'-'.time(). '.pdf')
