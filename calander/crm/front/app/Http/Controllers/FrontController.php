...
class FrontController extends Controller
{
    ...
    public function applyCoupon(Request $request){
        ...
        $cpn= Coupon::where('coupon_code',$allData['code'])->first();
 
        if($cpn){

            $uses= $cpn['uses'];
            $date="2020-04-25";
            $start= $cpn['start_date'];
            $end= $cpn['end_date'];

            $cpnCheck= CouponDetail::where('cpn_code',$allData['code'])->where('user_id',Auth::user()->id)->count();
            ...
            if($cpn->status ==0 && $uses !=""){
                //echo 'weq'.$cpnCheck.$uses;
                if($cpnCheck > $uses){
                echo 2;
                die;
                }
            ...
            if($cpnCheck > 0){
                //echo '>>k'.$cpnCheck;
                $chk= CouponDetail::where('orderId',$allData['orderId'])->where('user_id',Auth::user()->id)->first();
                if($chk){
                $chk->delete();
                }
                $cpnDetail= new CouponDetail();
                $cpnDetail->user_id= Auth::user()->id;
                $cpnDetail->orderId= $allData['orderId'];
                $cpnDetail->cpn_code= $allData['code'];
                $cpnDetail->cpn_type= $cpn['cupon_type'];
                $cpnDetail->cpn_price= $cpn['price'];
                $cpnDetail->save();
                echo 1;
    ...
    public function newsLetter(Request $request){
        $all= $request->all();
        //$data1=['email'=>'rahul@yopmail.com'];
        // return view('emails.contact.newsletter',compact('data1', $data1));die;
        ...
        if($all['checkbox']==1){
            $CheckNews= Newsletter::where('email',$all['email'])/*->where('subscription_type',1)*/->first();
            if($CheckNews->subscription_type==1){
                echo 1;
            }elseif($CheckNews->subscription_type==2){
                $CheckNews->subscription_type= 1;
                $CheckNews->update();
                echo 0;
            }else{
                ...
                $news->is_client= $check;
                $news->subscription_type= 1;
                $news->save();


               /* $data1=[];
                $userEmail = $all['email'];
                \Mail::send('emails.contact.newsletter',compact('data1', $data1), function($message) use ($userEmail)
                    {
    ...
    public function unsubscribe($email){
        ...
        return view('webpanel.front.unsubscribe')->with('email',base64_decode($email));
    }
    public function unsubscribeEmail(Request $request){
         
        $all= $request->all();
        $CheckNews= Newsletter::where('email',$all['email'])->first();
        if($CheckNews){
         $CheckNews->subscription_type= 2;
         $CheckNews->update();
         return back()->with('error','You have successfully unsubscribe');
        }else{

         
        }
        return redirect(url('/unsubscribe/'.base64_encode($all['email'])));
        //dd($all);
    }
