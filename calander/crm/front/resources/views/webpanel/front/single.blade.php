...
@section('description1', $product->seo_description)
@section('title1', $product->seo_title)
		...
		<div class="container">
			...
			<div class="col-md-12 single-right">
				<div class="col-md-5 single-right-left animated wow slideInUp" data-wow-delay=".5s">
					<div class="flexslider">
                        ...
                        @foreach($product->photos as $photos)
							...
							$fileName= $photos['media']['filename'] 
                             ...
                             <input type="hidden" id="main_image" value="https://crm.morettimilano.com/public/{{$folder.$fileName}}">
                             <li data-thumb="https://crm.morettimilano.com/public/{{$folder.$fileName}}" class="flex-active-slide">
								<div class="thumb-image"> <img class="main_image" src="https://crm.morettimilano.com/public/{{$folder.$fileName}}" alt="{{$product->name}}" data-imagezoom="true" class="img-responsive" onerror="this.onerror=null;this.src='{{Request::root()}}/public/front/images/no_image.jpg'"> </div>
							</li>
                        @endforeach
                        @endif                     
                        @if($multi_photo)
                          @foreach($multi_photo as $value)
                             <li data-thumb="{{IMGPATH}}/public/webshop_Images/{{$value->image}}">
								<div class="thumb-image"> <img src="{{IMGPATH}}/public/webshop_Images/{{$value->image}}" alt="<?php echo substr($value->image, 0, strpos($value->image, '.')); ?>" data-imagezoom="true" class="img-responsive" onerror="this.onerror=null;this.src='{{Request::root()}}/public/front/images/no_image.jpg'"> </div>
							</li>                   
                          @endforeach
                        @endif

                         @if(\App\Modules\Products\Variant::where('product_id',$product->id)->get())
                         <!-- {{\App\Modules\Products\Variant::where('product_id',$product->id)->get()}} -->
                          @foreach(\App\Modules\Products\Variant::where('product_id',$product->id)->get() as $image)
                          <!-- {{$image->image}} -->
                          @if(!empty($image->image))
                             <li data-thumb="{{IMGPATH}}/public/variant_images/{{$image->image}}" id="{{$image->id}}">
								<div class="thumb-image"> <img src="{{IMGPATH}}/public/variant_images/{{$image->image}}" alt="{{$image->image}}" data-imagezoom="true" class="img-responsive" onerror="this.onerror=null;this.src='{{Request::root()}}/public/front/images/no_image.jpg'"> </div>
							</li> 
							@endif                  
                          @endforeach
                        @endif
					...
					<!-- flixslider -->
						<script defer src="{{Request::root()}}/public/front/js/jquery.flexslider.js"></script>
						<link rel="stylesheet" href="{{Request::root()}}/public/front/css/flexslider.css" type="text/css" media="screen" />
						<script>
				...
				<input type="hidden" name="id" value="{{ encryptIt($product->id) }}">
                                    ...
                                    @if($variants->count() > 0)
                                        ...
                                        @foreach($variants as $variant)
                                           ...
                                           <!--  <a class="color-box" style="background:{{ @$variant->color->hex_code }}"
                                               ...
                                               <option variant_id1="<?php if(!empty($variant->image)) {echo $variant->id;}else{echo '';}?>" data-stock-label="{{ $variant->qty > 0 ? 'In Stock' : 'Out Of Stock' }}"
                                               data-id="{{ $variant->id }}" title="{{ @$variant->color->name }}" 
                                               data-qty="<?=$variant->qty?>" value="" style="background-color: {{ @$variant->color->hex_code }};color:<?php if($variant->color->name == "Rosa" || $variant->color->name == "Beige" || $variant->color->name =='Silver Metal'){ echo 'black'; }else{ echo 'white'; } ?>;">{{ @$variant->color->name }}</option>
                                        @endforeach
                    ...
                    <a class="item_add btn btn-primary" href="javascript:;" data-toggle="modal" data-target="#stockStatus" data="{{$product->name}}" data-id="{{encryptIt($product->id)}}">Stock Status</a>
...
<style>
 ...
 <div class="container">
  ...
  <div class="modal fade" id="stockStatus" role="dialog">
    ...
    <tbody>
      ...
      <tr>
        <td>{{ @$variant->color->name }}<a class="color-box" style="background:{{ @$variant->color->hex_code }}"
                                               data-stock-label="{{ $variant->qty > 0 ? 'In Stock' : 'Out Of Stock' }}"
                                               data-id="{{ $variant->id }}" title="{{ @$variant->color->name }}" data-qty="<?=$variant->qty?>"></a></td>
        <td>{{$variant->qty}}</td>
        <td>
        	@if($variant->qty > 0)
 ...
 <!--Model-->
	<script src="{{Request::root()}}/public/front/js/imagezoom.js"></script>
