@extends('pdf.viewbase')
@section('body')

    <div style="text-align: center">
        <span style="font-size:35px;font-weight:bold;">MORETTI MILANO</span>
    </div>

    <br/> <br/>
    @if($order->creator)
        <table width="98%" cellspacing="0" cellpadding="4" border="0">
            <tr>
                <td width="70%">
                    @if($order->creator->parent)
                        Moretti Milano <br/>
                        Lille Kongensgade 14, 1 sal <br/>
                        1074 Copenhagen K <br/>
                        Denmark <br/>
                        <br/>
                        VAT nr: 32048366 <br/>
                        Phone: +4522323640 <br/>
                        Bank: Jyske Bank: 7851 1265999 <br/>
                        IBAN : DK5378510001265999 <br/>
                        SWIFT: JYBADKKK <br/>
                        E-mail: info@morettimilano.com <br/>
                        Webside: www.morettimilano.com <br/>
                    @endif
                </td>
                <td width="30%" valign="middle" align="left">
                    @if(@$order->creator)
                        {{ @$order->creator->fullName() }} <br>
                        {{ @$order->creator->address }} <br>
                        {{ @$order->creator->zipcode }} {{ @$order->creator->city }} <br/>
                        {{ @$order->creator->country }} <br/>
                        <br/>
                        Phone: {{ @$order->creator->phone }} <br>
                        VAT nr: {{ @$order->creator->vat_number }}
                        E-mail: {{ @$order->creator->email }} <br>
                    @endif

                    Order Date: {{ $order->createdDate('d/m/Y') }}
                    @if($order->hasExpectedShippingDate())
                        <br>
                        <strong>Expected Shipping Date : </strong>
                        {{ $order->expected_shipping_date->format("d/m/Y") }}
                    @endif
                </td>
            </tr>
        </table>

        <br/><br/>

        <table width="98%" cellspacing="0" cellpadding="4" border="1">
            <thead>
            <tr>
                <th>SN</th>
                <th>Product Name</th>
                <th>Color</th>
                <th>Qty</th>
                <th>Price</th>
                <th>Discount</th>
                <th>Total</th>
            </tr>
            </thead>
            <tbody>
            <?php $items = $order->items()->with('product.photos')->get(); $grandTotal = 0; ?>

            @foreach($items as $k => $item)
                <tr>
                    <td>{{ $k + 1 }} </td>
                    <td>{{ @$item->product->name }}</td>
                    <td>{{@$item->variant->color->name }}&nbsp;&nbsp;@if($item->note!='')Note:{{$item->note}}@else
                                @endif</td>
                    <td align="right"> {{ $item->qty }} </td>
                    <td align="right">
                    {{currency(number_format($item->price/euro()->conversion,2))}}
                    </td>
                    <td align="right"> {{ $item->discount }} %</td>
                    <?php $total = $item->getDiscountedPrice() * $item->qty; $grandTotal += $total; ?>
                    <td align="right"> {{currency(number_format($total/euro()->conversion,2))}}</td>
                </tr>
            @endforeach
            </tbody>
            <tfoot>
            <tr>
                <th colspan="6" style="text-align: right;">Total</th>
                <th align="right">
                   
                    €{{number_format($grandTotal/euro()->conversion,2)}}
                </th>
            </tr>
            @if($order->tax_percent > 0)
                <?php
                $taxPercent = $order->tax_percent;
                ?>
                <tr>
                    <th colspan="6" style="text-align: right;">Tax</th>
                    <th align="right">
                        {{ $taxPercent }}% (€{{number_format($grandTotal * ($taxPercent / 100)/euro()->conversion,2)}})
                        
                    </th>
                </tr>
                <?php
                $grandTotal += ($grandTotal * ($taxPercent / 100));
                ?>
            @endif

            @if($order->toRefund)
                <?php $refund = $order->toRefund->getRefundAmount();
                $grandTotal -= $refund;
                ?>
                <tr>
                    <th colspan="6" style="text-align: right;">Refund Deduction</th>
                    <th align="right">
                        {{ currency($refund) }}
                        {{currency(number_format($refund/euro()->conversion,2))}}
                    </th>
                </tr>
            @endif
            <tr>
                <th colspan="6" style="text-align: right;">Shipping Price</th>
                <th align="right">
                    
                     {{currency(number_format($order->shipping_price/euro()->conversion,2))}}
                </th>
            </tr>
            @if($order->getCreditNoteAmount() > 0)
                <tr>
                    <th colspan="6" style="text-align: right;">Kreditnota</th>
                    <th align="right"> 
                     {{currency(number_format($order->getCreditNoteAmount()/euro()->conversion,2))}}
                    </th>
                </tr>
            @endif
            <?php

            $chk= \App\Models\CouponDetail::where('orderId',$order['OID'])->first();
            // echo '<pre>';print_r($chk);die;
            ?>

            @if($chk)

            <?php

            $check= 0;
            if($chk->cpn_type=='fixed'){
                $act= number_format(($order->getTotalPrice() - $order->getCreditNoteAmount())/euro()->conversion,2);
                $actamt= $act-$chk->cpn_price;
            }elseif($chk->cpn_type=='percent'){

                $act= number_format(($order->getTotalPrice() - $order->getCreditNoteAmount())/euro()->conversion,2);

                $percent= $act*$chk->cpn_price/100;
                $actamt= $act-$percent;
                 

                $check=1;

            }else{
                $actamt= number_format(($order->getTotalPrice() - $order->getCreditNoteAmount())/euro()->conversion,2);
            }
            

            ?>
             <tr>
                <th colspan="6" style="text-align: right;">Coupon Discount</th>
                <th align="right">
                    @if($check==1)
                    ({{$act}} * {{ $chk->cpn_price }}%)  {{currency($percent)}}
                    @else
                    {{currency($chk->cpn_price)}}
                    @endif
                </th>
            </tr>

            @endif

            <tr>
                <th colspan="6" style="text-align: right;">Grand Total</th>
                <th align="right">
                     
                    {{currency($actamt)}}
                </th>
            </tr>
            </tfoot>
        </table>

    @else
        <h3>INVALID ORDER</h3>
    @endif

@stop
