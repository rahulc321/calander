...
@section('body')
    ...
    <div class="content">
        ...
        <form method="post" action="{{ sysRoute('creditnotes.update', encryptIt($creditNote->id)) }}"
            ...
            <div class="well">
                <div class="form-group">
                    ...
                    <div class="col-md-5">
                        {{ $creditNote->user ? $creditNote->user->fullName() : '' }}
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label col-md-2" for="note">Note (Memo):</label>
                    <div class="col-md-5">
                        <textarea class="form-control" id="note" name="note">{{ $creditNote->note }}</textarea>
                    </div>
                </div>
            </div>
                        ...
                        <tr>
                            <td>
                                <select name="product_id[]" class="form-control blank product" required
                                    ...
                                    {!! OptionsView(\App\Modules\Products\Product::all(), 'id', 'name', $item->product_id) !!}
                                </select>
                            </td>
                            <td>
                                <select name="variant_id[]" class="form-control variant empty" required>
                                    <option value="">Choose</option>
                                    {!! OptionsView($item->product->variants, 'id', function($i){
                                        return @$i->color->name;
                                    }, $item->variant_id) !!}
                                </select>
                            </td>
                            <td>
                                <input type="number" name="qty[]" class="form-control blank" value="{{ $item->qty }}" required>
                            </td>
                            <td>
