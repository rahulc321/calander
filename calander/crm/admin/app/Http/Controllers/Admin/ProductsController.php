...
class ProductsController extends Controller
{
    ...
    {   
        ...
        $data->web_shop_price = Input::get('webshop_price');
        $data->seo_title = Input::get('seo_title');
        $data->seo_description = Input::get('seo_description');
        $data->seo_keywords = Input::get('seo_keywords');
        $data-> save();
        $total= count($_FILES['web_photo']['name']);
        for( $i=0 ; $i < $total ; $i++ ) 
          {
            $tmpFilePath = $_FILES['web_photo']['tmp_name'][$i];
            if($tmpFilePath != ""){
                $newFilePath = public_path().'/webshop_Images/';
                // echo $newFilePath;
                if(move_uploaded_file($tmpFilePath, $newFilePath.$_FILES['web_photo']['name'][$i])) {
                $multi_photo= new ProductImage;
                $multi_photo->product_id= $product['id'];
                $multi_photo->image= $_FILES['web_photo']['name'][$i];
                $multi_photo-> save();
              }
            }
          }
    ...
    public function edit($id)
    {
        ...
        $multi_photo = ProductImage::where('product_id',$id)->get();
        return sysView('products.edit', compact('product','multi_photo'));
    }
  ...
  //18-09-2019
    ...
    public function deleteImage(){
       $data= ProductImage::where('id',$_POST['id'])->first();
       $data->delete();
       echo 1;
    }
    ...
    public function getOrders()
    {
        ...
        $totalPrice = $this->products->getTotalCount(Input::all(), 10);

        // $products1 = Product::all();
        // echo '<pre>';print_r(count($products1));die;
        return sysView('products.orders', compact('products'))->with('totalprice',$totalPrice);
    }
