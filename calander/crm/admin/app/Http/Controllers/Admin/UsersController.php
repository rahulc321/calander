...
use App\Modules\Coupon;
use App\Modules\Newsletter;
use App\Modules\Newstemplate;
use App\Modules\Newsletteremail;
use App\Modules\Website_info;
use Carbon\Carbon;
...
class UsersController extends Controller
{
    ...
    public function store()
    {
        ...
        $allData = Input::all();
        $map_status = $allData['map_status'];

        $user = $this->users->createUser(Input::all());

        \DB::update('update users set map_status=? where id = ?',[$map_status,$user->id]);
    ...
    public function update($id)
    {
        ...
        $allData = Input::all();
        $userAddress = $allData['address'].' '.$allData['city'].' '.$allData['country'];
        $map_status = $allData['map_status'];
        $address1 = str_replace(' ', '%20', $userAddress);
        $url='https://maps.googleapis.com/maps/api/geocode/json?address='.$address1.'&key=AIzaSyDe6orIFBWaj-afNJA8ma4u0TbNjDQGJn4';
        $api = file_get_contents("$url");
               
        $buffer = json_decode($api,true);

        if($buffer['status']=='OK'){
        $lat=$buffer['results'][0]['geometry']['location']['lat']; 
        $lng=$buffer['results'][0]['geometry']['location']['lng']; 
        ...
        if ($this->users->updateUser($id, Input::all())) {
            if($buffer['status']=='OK'){
                \DB::update('update users set lat = ?,lng=?,map_status=? where id = ?',[$lat,$lng,$map_status,$id]);  
            } else {
                \DB::update('update users set map_status=? where id = ?',[$map_status,$id]);  
            }
    ...
    public function automaticEmail(){
       // echo 'Hey';die;
        ...
        public function viewMap(){
            ...
            $clent= \DB::table('future_client')->get();;
            //  echo '<pre>';
            // print_r($clent);
            //  die;
            $dData= array_merge($userInfos,$clent);
            // echo '<pre>';
            // print_r($dData);
            // die;
            return view('webpanel.client.map')->with('userInfos',$dData);

        }
        public function futureClient(){
            $clent= Client::get();
            return view('webpanel.client.index')->with('clent',$clent);
        ...
        public function addClient(Request $request){
            ...
            if(!empty($data)){
                ...
                $client->lng= $lng;
                $client->type= 'future';
                $client->map_status= $map_status;
                $client->save();
                 \Session::flash('success', 'You have successfully Added!');
                }
                return view('webpanel.client.add-client');
        ...
        public function delete($id){
            ...
            return redirect('webpanel/future-client');

        }

        public function clientEdit($id){
            $client = Client::where('id',$id)->first();
            return view('webpanel.client.add-client')->with('client',$client);
        }
        public function updateClient(Request $request){
                ...
                return redirect('webpanel/clien-edit/'.$cid);
        ...
        public function updateAll(){
           ...
           $userInfos = \DB::table('users')
            ...
            ->get();
            $i=1;
            foreach($userInfos as $allData){
                // echo '<pre>';print_r($allData->address);
                // die;
                $userAddress = $allData->address;
                $address1 = str_replace(' ', '%20', $userAddress);
                $url='https://maps.googleapis.com/maps/api/geocode/json?address='.$address1.'&key=AIzaSyDe6orIFBWaj-afNJA8ma4u0TbNjDQGJn4';
                $api = file_get_contents("$url");
                ...
                \DB::update('update users set lat = ?,lng=? where id = ?',[$lat,$lng,$allData->id]);
                // if($i==3){
                //     break;
                // }
        ...
        // website info
          ...
          if(!empty($data)){
             $updat= Website_info::where('id',$data['info_id'])->first();
             if($data['info_id']== 7){
             $updat->info = $data['info'];
             $updat->update();
             $data1= Website_info::all();
              return view('webpanel/websiteInfo/website-info')->with('data',$data1)->with('success1','updated successfully!');
             }
             else{
             $updat->info = $data['info'];
             $updat->update();
             ...
             return view('webpanel/websiteInfo/website-info')->with('data',$data1);
             }
          } 
          else{ 
            $data2= Website_info::all();
            return view('webpanel/websiteInfo/website-info')->with('data',$data2);
        }
        }


        public function enquiry(){
            $enquiry = \DB::table('product_enquiry')->orderBy('id', 'DESC')->get();
           return view('webpanel.enquiry.index')->with('enquiry',$enquiry);
        } 

        // for coupon code only


        public function listCoupon(){
            $coupons= Coupon::orderBY('id','desc')->get();
            return view('webpanel.coupon.index')->with('coupons',$coupons);
        } 
         ...
         public function addCoupon(Request $request){
            ...
            if(!empty($data)){
                 ...
                 if($coupon){
                    ...
                    return view('webpanel.coupon.add-coupon');
                 }else{
        ...
        public function couponEdit($id){
            ...
            return view('webpanel.coupon.edit-coupon')->with('edit',$edit);
        }
        // Update coupon
        ...
        public function listNewsletter()
        {
            ...
            return view('webpanel.newsletter.index')->with('newsletter',$newsletter);
        }
        public function addNewsletter(Request $request)
        {
            ...
            if($input){
                ...
                if($newsletter){
                     \Session::flash('error', 'Coupon Code already Exists!');
                    return view('webpanel.newsletter.add-newsletter');
                }else{
                    $newsletter= new Newsletter();
                    $newsletter->email=$input['email'];
                    $newsletter->is_client=1;
                    $newsletter->subscription_type=1;
                    $newsletter->save();
                     \Session::flash('success', 'You heve Successfully Add Coupon!');
                    return redirect('/webpanel/list-newsletter');
                }
        ...
        public function unsubscribe($id)
        {
            ...
            if($newsletter->subscription_type==2){
            $newsletter->subscription_type =1;
            $newsletter->update();
            }elseif($newsletter->subscription_type==1){
                $newsletter->subscription_type =2;
                $newsletter->update();
            }


             \Session::flash('success', 'You heve successfully update User!');
                    return redirect('/webpanel/list-newsletter');
        }

        public function sentEmail(){
            $templates= Newstemplate::get();
            $newsletter= Newsletter::get()->toArray();
           // echo '<pre>';print_r($newsletter);die;
            return view('webpanel.newsletter.sent-email')->with('templates',$templates)->with('newsletter',$newsletter);
        }

        public function sentEmailTo(Request $request){
            ...
            if($input['template']==""){
                 \Session::flash('error', 'Please select template first');
                return redirect('/webpanel/sent-email');

            }elseif($input['message'] ==""){
                 \Session::flash('error', 'Message field is required');
                 return redirect('/webpanel/sent-email');

            }elseif(count($input['email']) == 0){
                \Session::flash('error', 'Please select newsletter user first');
                 return redirect('/webpanel/sent-email');
            ...
            foreach ($input['email'] as $key => $value) {
                $newsEmail= new Newsletteremail();
                $newsEmail->template_id=$input['template'];
                $newsEmail->newsletter_id=$value;
                $newsEmail->message=$input['message'];
                $newsEmail->save();
                # code...
                //echo $key;
                $template="";
                if($input['template']==1){
                    $template= 'template1';
                }elseif($input['template']==2){
                    $template= 'template2';
                }elseif($input['template']==3){
                    $template= 'template3';
                }

                $data1=['message'=>$input['message']];
                return view('emails.newsletter.'.$template,compact('data1', $data1));die;

                $newsletter= Newsletter::where('id',$value)->first()->toArray();
                //echo $newsletter['email'];echo '<br>';


                 $userEmail = $newsletter['email'];
                \Mail::send('emails.newsletter.'.$template,compact('data1', $data1), function($message) use ($userEmail)
                    {
                        $message->to($userEmail);
                        $message->subject('Message from Newsletter');
                        $message->bcc('sonu@yopmail.com');
                        //$message->bcc('morettimilano@gmail.com');
                        //$message->attachData($pdf->output(), 'Invoice.pdf',['mime' => 'application/pdf']);
                    });
            ...
            \Session::flash('success', 'You heve successfully Sent Email!');
        return redirect('/webpanel/sent-email');
        }
