...
class InvoicesController extends Controller
{
    ...
    public function getToggleStatus($id, $status)
    {
        ...
        foreach($items as $item){
            ...
            if($status==1 || $status==2){
                ...
                if($variant->qty > 0){
                    $tqty=  $item->qty; 
                    $variant->qty = ($variant->qty-$tqty);
                    $variant->update();

                }else{
                    $variant->total_required = $item->qty;
                    $variant->update();
                }
                ...
                if($variant->total_required > 0){
                    //echo 'hi';die;
                   $tqty=  abs($variant->total_required-$item->qty);
                   $variant->total_required = $tqty;
                   $variant->update();
                }else{
                    //echo $variant->qty.'>'.$item->qty;die;
                    $tqty=  $item->qty; 
                    $variant->qty = ($variant->qty+$tqty);
                    $variant->update();
                }
    ...
    public function getDownload(PdfExportService $exportService, $id)
    {
        ...
        $invoice= Invoice::where('IID',$IId)->first();
